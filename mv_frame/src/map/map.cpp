...
namespace myslam
{
    ...
    void Map::InsertKeyFrame(Frame::Ptr frame)
    {
        ...
        {
            keyframes_.insert(make_pair(frame->keyframe_id_, frame)); // 如果id没用过，插入进hash表
            active_keyframes_.insert(make_pair(frame->keyframe_id_, frame));
        }
        else
        {
            keyframes_[frame->keyframe_id_] = frame; // 如果id用过，说明hash表已存在这一条目，可能被弃用了所以id被放出来，直接进行覆盖操作
